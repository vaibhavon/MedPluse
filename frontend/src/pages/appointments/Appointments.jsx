...
export default function Appointments() {
  ...
  const today = useMemo(() => getDateOnly(todayDate), [todayDate]);
  const tomorrow = useMemo(() => {
    const t = new Date(todayDate);
    t.setDate(t.getDate() + 1);
    return getDateOnly(t);
  }, [todayDate]);
  ...
  const handleSave = () => {
    ...
    if (editing) {
      ...
      idRef.current++;
      addAppointment({ id: "APT" + idRef.current, ...form });
    }
  ...
  return (
    <div className="appointments">
      ...
      {/* SECTION TITLE */}
      <h3 className="section-title-appoi">{getSectionTitle()}</h3>
      ...
      <div className="appointments-list">
        ...
        {filteredAppointments.map(a => (
          <div key={a.id} className="appointment-people">
